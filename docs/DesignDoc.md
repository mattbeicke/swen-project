...
During our first iteration of static code analysis, we had approximately 250 issues which was whittled down to 12 in Sprint 3.
...
One design improvement our team would explore given additional time is refactoring the CompletedNeeds system. It currently is included within the CupboardController, but the CupboardController class depends on many unrelated pieces of data which could be separated if care is taken. There is also one other warning given by SonarQube, that being to "make the enclosing method 'static'", which we were unsure how to solve as official templates seem to also include a similar issue. 
